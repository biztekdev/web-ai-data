...
async function main() {
    try {
        // Read the finishes entity file
        const entityData = JSON.parse(fs.readFileSync('./entities/finishes.json', 'utf8'));
        ...
        if (success) {
            console.log('Finishes entity imported successfully.');
        } else {
            console.error('Failed to import finishes entity.');
            process.exit(1);
        }
